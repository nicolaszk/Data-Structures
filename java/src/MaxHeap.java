...
public class MaxHeap {
    ...
    public void bubbleUp(int index){
        ...
        if(index > 0 && heap[index] > heap[parent]){ // if value is greater than parent, swap and continue bubbling up
            swap(index, parent);
    ...
    public void bubbleDown(int index){ // bubble down: swap with smaller child until heap property is restored
        ...
        if(largest != index){ // if changed, swap and continue bubbling down
            swap(index, largest);
            bubbleDown(largest);
        }
    }
